...
def merge_clean_data():
    for fields in mappings:
        ...
        if fields["format"] == "ID":
            ...
            wrong_syntax = ["MÃ©xico", "EspaÃ±a",
                            "Estados Unidos de AmÃ©rica", "HaitÃ­"]
            ...
            catalog = catalogs[fields["format"]]
            covid_df[field].replace(
                catalog["CLAVE"].values, catalog["DESCRIPCIÓN"].values, inplace=True)
...
print("The data set contains " +
      str(sex_by_state_df.shape[0]) + " rows by " + str(sex_by_state_df.shape[1]) + " columns.")
